...
User.getFeedFromUser = function(iduser){
    return new Promise(function(resolve, reject) {
        ...
        (select count(idpost) from upvotes u where u.idpost = post.idpost) as postNum, 
        (select g.name from databettingspree.group g where g.idgroup = post.idgroup) as namegroup, (select state from bet where post.idbet = idbet) as state 
        from follower follower, post post, user u 
